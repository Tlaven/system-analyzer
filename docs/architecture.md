...
| **modal** | `src/modal.js` | `showModal`(通用模态对话框 UI 原语,v0.13 从 input.js 抽出以斩断 panel→input 反向 import) |
...
每条 `{ target, description, transform? }`,`target` 指向另一实例的 attrs,`transform` 是可选 JS 语句片段(ADR-003,边级响应式公式)。一个实例可以有任意多条 edges(多对一 / 多对多 / 同目标多边都支持)。bootstrap 里写 `X.edges = [{ target: Y, description: '...' }]` 即建立一条纯结构边;加 `transform: "target['Y'] = source['X'] * 0.02"` 即附上轻量响应式公式。
...
源实例的 `attrs.edges` 数组含 `{ target, description }` 条目,且 `target` 是另一 inst.attrs(含 `__instId` 反查)。`deriveEdges(state)` in `src/codegraph.js` 遍历所有实例的 `attrs.edges` 派生边视图(v0.13 从 io.js 迁入 + lazy 缓存:dirty flag + `invalidateEdges()` 失效,7 处写边入口 + runSource 自动调失效)。边 id = `<srcVar>><tgtVar>>idx`(idx 是 attrs.edges 数组里的位置,区分同对多边)。
...
UI 模式交互:
  用户操作(panel 加属性 / 拖边 / Ctrl+V ...)
    ...
    → triggerPropagate(varName) debounce 300ms:
        execMode='auto' → propagate() 含 evalTransforms()
        off / manual   → runTransforms() 只跑 transform(ADR-003 OQ#2:transform 像 Excel formula)
    → render()
...
**实例虚拟字段(向后兼容)。** `wrapInstance(inst)` in `src/io.js` 给每个 RuntimeInstance 加 getter:`id`/`classId`/`label`/`description`/`name`/`x`/`y`/`properties`/`error`。`x`/`y` 实际存取于 `state.visualState.positions[varName]`。`properties` 是合并视图(`{...cls.attrs, ...inst.attrs}` 减 `{name, description, edges}`)。v0.13 删除了 v0.8 残留的 `inputs`/`outputs`/`computed` 三个 0-调用方 getter(端口概念已被四边柄系统取代,详见 `docs/edge-routing.md`)。
...
### transform 表达式不变量(ADR-003)

7. **transform 是 `attrs.edges[i].transform` 上的可选 JS 语句片段字符串**。求值方式 `new Function('source','target', body).call(null, srcAttrs, tgtAttrs)`。无 transform 的边是纯结构边,不保留任何隐式"默认流动"。
8. **属性访问一律 bracket access**(`source['总人数']` / `target['需求量']`)。不用点访问——理由:(a) 消除"中文 key 能否点访问"歧义;(b) 跟 panel 属性列表(字符串)视觉一致;(c) 跟 JSON 序列化对齐;(d) UX 上用户照抄属性名字符串。
9. **表达式只能引用边两端 attrs**(source / target)。不扩展 topo sort——边本身已经在依赖图里,source 先于 target,顺序天然正确。跨节点引用是罕见场景,真要表达请手补边。
10. **transform 不受 `execMode === 'off'` 抑制**。`panel.js:triggerPropagate` 改为:`auto` 模式走 `propagate()`(含 `evalTransforms`),`off` / `manual` 走 `runTransforms()` 绕过短路。理由:transform 像 Excel formula,是 attrs 模型的一部分,不是"外部方法体",用户写了就该立即生效。
11. **`evalTransforms()` 不走 topo 序也不跳过环**。遍历所有 `runtimeInstances` 的 `attrs.edges`,有 transform 的边各跑一次。声明式公式不会无限循环(区别于方法体可能产生的循环依赖)。

### scanner 不变量

12. **scanner 只读 `new cls()` 实例上的 3 个 class field**(description / name / attrs)。**不**扫 static、**不**扫 constructor、**不**扫 class.edges(v0.8 残留)。scanner 还会从 attrs 里过滤掉误写的 `edges` 键。
13. **`GraphStarter.add` 只 `deepCopy(fresh.attrs)`** 不预填其他字段。实例 attrs 真正可追加(包括 `edges`)。

### 数据流不变量

14. **UI 模式:用户操作 → mutate runtimeInstances → `syncCodeFromRuntime()` → `save()` → `render()`**。四步顺序不可乱(尤其是 syncCodeFromRuntime 必须在 save 前)。
15. **Code 模式:用户输入 → debounce 400ms → `commitCode()` → `runSource` + `save` + `render()`**。debounce 防抖是硬约束。

### 持久化不变量

16. **`sa_data.version` 必须是 6**。其他版本(v0.5/v0.6/v0.7/v0.8)硬切换丢弃。
17. **URL hash 上限 24000 字符**(UTF-8 safe base64)。超限不分享。
